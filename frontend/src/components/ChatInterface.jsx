...
const ChatInterface = ({ minimized = false }) => {
  ...
  return (
    <div className={`flex flex-col bg-white dark:bg-slate-800 transition-colors ${minimized ? 'h-[600px] w-[450px] rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700' : 'h-full'}`}>
      {/* Header */}
      ...
      {/* Messages */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50 dark:bg-slate-900/50 transition-colors min-h-0">
        {messages.map((msg, idx) => (
          ...
          >
            <div className={`max-w-[75%] rounded-2xl p-4 text-sm shadow-sm leading-relaxed transition-colors ${msg.role === 'user'
